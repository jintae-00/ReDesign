#!/usr/bin/env python3
"""
figma_metrics.py — Element-level evaluation metrics for the Figma dataset.

This is a LIBRARY of metric functions (element extraction, GT↔prediction
matching, visual-quality / panoptic / composite metrics) imported by the
evaluation entry-point scripts:
    evaluation/eval_accuracy_baselines_figma.py
    evaluation/before_eval_editability_precompute_matches.py

Run those scripts (not this file) to evaluate. They take the flat dataset via
``--figma-data figma_data`` and the agent's inference output via ``--agent-dir``;
there is no dataset-split concept.
"""
