...
def run(
    ...
    shears: Sequence[float] = (0.25, 0.4, 0.55),
    ...
    candidates = build_style_candidates(pairs, "text_italic", [{"shear": float(v)} for v in shears])
    results = evaluate_style_subtask(
        candidates,
        cache,
        seed=seed,
        max_tasks=max_tasks,
        progress_prefix=f"[{model}][style_italic]",
        log_every=log_every,
        save_pair_viz_dir=(output_dir / model / "style_italic" / "element_pairs") if save_pair_viz else None,
        pair_viz_max=pair_viz_max,
        reference_results=reference_results,
        num_workers=num_workers,
        show_tqdm=show_tqdm,
        checkpoint_path=output_dir / model / "style_italic_results.json",
        resume=True,
    )
    return {
