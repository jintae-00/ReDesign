#!/usr/bin/env python3
"""Save GT:Agent greedy match pairs for editability tasks."""
...
from evaluation.editability_utils.match_runner import run_matching
from evaluation.editability_utils.matching_core import MatchConfig


def main() -> None:
    parser = argparse.ArgumentParser(description="GT:Agent matching saver (editability)")
    ...
    print(f"[DONE] agent matching episodes={summary['num_episodes']} -> {Path(args.output) / 'agent'}")
