...
def find_flat_color_region(
    ...
    for color, _color_mask in zip(cc_palette, cc_color_masks):
        # Filter out color components that do not overlap with the mask region.
        # Use label-based filtering instead of iterating over each CC individually,
        # which is O(H*W) instead of O(N_components * H * W).
        num_labels, labels = cv2.connectedComponents(_color_mask.astype(np.uint8), connectivity=8)
        if num_labels <= 1:
            continue
        # Find which labels overlap with mask
        overlapping_labels = np.unique(labels[mask])
        overlapping_labels = overlapping_labels[overlapping_labels > 0]  # exclude bg label 0
        if th_overlap_ratio > 0:
            # For each overlapping label, check overlap ratio
            keep_labels = []
            for lab in overlapping_labels:
                c = labels == lab
                if (c & mask).sum() / c.sum() > th_overlap_ratio:
                    keep_labels.append(lab)
            overlapping_labels = np.array(keep_labels, dtype=overlapping_labels.dtype)
        if len(overlapping_labels) == 0:
            continue
        _color_mask_filtered = np.isin(labels, overlapping_labels)
