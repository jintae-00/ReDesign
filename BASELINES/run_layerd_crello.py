#!/usr/bin/env python3
"""
run_layerd_crello.py - LayerD + LaMa Iterative Extraction on Crello Dataset
...
def setup_logging(log_file: Path, name: str = "layerd_crello") -> logging.Logger:
...
def is_record_failed(output_dir: Path, record_id: str) -> bool:
    return (output_dir / record_id / "_FAILED").exists()


def mark_record_failed(output_dir: Path, record_id: str, reason: str):
    fail_dir = output_dir / record_id
...
def worker_process(
    ...
    worker_logger = setup_logging(
        ...
        name=f"layerd_crello_w{worker_id}"
    )
    ...
    while True:
        ...
        try:
            ...
            # Copy input image
            input_dest = record_output_dir / INPUT_IMAGE_NAME
            ...
            create_reconstructions(record_output_dir, worker_logger)
            ...
            worker_logger.info(
                f"  Done: {record_id} | {pipeline_result['num_layers']} layers | "
            ...
            worker_logger.error(f"  TIMEOUT: {record_id} after {elapsed:.0f}s -- marked as failed, skipping")
            ...
            if "CUDA out of memory" in err_str or "CUDA error" in err_str:
                mark_record_failed(output_dir, record_id, f"CUDA error: {err_str[:200]}")
                worker_logger.error(f"  CUDA FAIL (permanent): {record_id}: {err_str[:200]}")
            result_queue.put({
                "worker_id": worker_id,
                "record_id": record_id,
                "success": False,
            ...
            worker_logger.error(f"  Failed: {record_id}: {e}")
...
def run_layerd_crello(
    ...
    logger.info(f"Will process {len(records_to_process)} records (skipped {skipped} completed, {skipped_failed} failed)")
    ...
    for rec in records_to_process:
        frame_queue.put((rec.record_id, str(rec.image_path)))
    ...
    pbar = tqdm(total=total, desc="LayerD Crello", unit="record", ncols=100)
