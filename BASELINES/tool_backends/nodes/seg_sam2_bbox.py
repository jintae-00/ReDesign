 # BASELINES/tool_backends/nodes/seg_sam2_bbox.py
...
SEG_MASK_MIN_PX = 8  # unified threshold
...
def node(state: GraphState) -> Dict[str, Any]:
    ...
    if "boxes" not in args:
        args["boxes"] = (state["_runtime"]["detect"].get("boxes") or [])
    if "det_ids" not in args:
        args["det_ids"] = (state["_runtime"]["detect"].get("det_ids") or [])

    out = _call_tool("SAM2_BBOX", args, state)  # {"mask_union": ..., "masks_by_id": {...}}
    ...
    saved = None
    if union:
        extra, saved = r_save_artifact(union, "sam2_bbox", "segment", state)
        pieces.append(extra)
    ...
    if area < SEG_MASK_MIN_PX:
        return r_abort_to_verify_sequence(
            state, 
            node="sam2_bbox", 
            error_msg=f"sam2_bbox segmentation failed: segmentation mask area too small (area={int(area)})",
            details={"mask_area": int(area), "threshold": int(SEG_MASK_MIN_PX)},
        )

    # Normal path: a single update_segment reflects segment + img["seg"] + base_img at once
    pieces += [
        r_update_segment(
            "sam2_bbox", saved, state,
            masks_by_id=by_id,
