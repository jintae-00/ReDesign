...
def node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # 항상: 새 시퀀스 시작(+seq_id 증가/디렉토리 생성) + _runtime 강제 초기화(교체)
    init = r_reset_runtime_for_next_seq(state)

    # LLM 프롬프트에서 사용할 base_pre
    ...
    # ★ 현 시점까지 학습된 Tool Sequence Few-shot Heuristics 스냅샷 읽기
    ...
    # --- Planner 출력: { "used_heuristics": [...], "actions": [...] } 기대 ---
    ...
    # used_heuristics 를 기반으로 ref_count 증가
    '''
    if used_heuristics:
    ...
    # (A) LLM이 준 순수 action 리스트에서 trajectory만 먼저 추출
    ...
    # tail은 metrics_gate만 추가 (이후 라우팅은 각 노드에서 명시적으로 수행)
