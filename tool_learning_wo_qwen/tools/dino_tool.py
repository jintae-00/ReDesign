...
def _apply_containment_filter(
    ...
    """
    큰 박스 A가 작은 박스 B를 contain_ratio 이상 포함하면 A 제거.
...
def run_dino_batch_all(image_path: str,
                       ...
                       *,
                       ...
                       contain_ratio_thresh: float = 0.9  # A가 B를 이 비율 이상 포함하면 A 제거
                       ):
    """
    여러 라벨 각각에 대해 `predict_filtered_boxes_matching` 를 반복 호출하여
    score_min_ratio(=score_min) 를 통과한 모든 박스를 모아 반환한다.

    현재는:
      - area_max_ratio 기반 면적 필터: `predict_filtered_boxes_matching` 내부에서 비활성화
      - label별 top-k 선택: 비활성화 (여러 박스를 동시에 허용)
      - Containment Filter: 비활성화 (drop_containers는 향후 재활성화를 위한 자리)
    """
    ...
    for lab in labels:
        ...
        # (3) top-k per label 활성화
        # 상위 k 인덱스 (최소 1, 최대 size)
    ...
    # img_cv 텐서 + PIL 이미지 해제
    del pil, img_cv

    # ===== Containment Filter 비활성화 =====
