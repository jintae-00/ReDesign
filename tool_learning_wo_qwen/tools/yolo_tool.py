# src/langgraph/tools/yolo_tool.py
...
def _apply_containment_filter(
    ...
    """
    큰 박스가 작은 박스를 contain_ratio 이상 포함하면 큰 박스를 제거.
...
def run_yolo(image_path: str,
    ...
    """
    YOLO 기반 범용 object/picture 감지.
    returns:
      {
        "boxes":  [[x1,y1,x2,y2], ...],   # 최종 필터링된 bbox
        "confs":  [float, ...] or None,   # 최종 bbox 에 대응하는 score
        "labels": [str, ...],             # human-readable label
        "viz":    "<path/to/viz.png>",    # 항상 'boxes' 에 맞춰 다시 그린 viz
      }
    """
    ...
    # threshold 이후 박스가 없다면: viz에는 아무 bbox도 없어야 함
    if len(bboxes) == 0:
        _draw_boxes(image_path, [], [], [], vis_path)
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
    ...
    return {
        "boxes": boxes,        # ← viz 에 그려진 bbox 와 1:1 대응
        "confs": confs,
        "labels": labs,
        "viz": str(vis_path),
    }
