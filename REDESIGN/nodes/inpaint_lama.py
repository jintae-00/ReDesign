# REDESIGN/nodes/inpaint_lama.py
"""
LaMa Inpainting Node - Removes objects/text using LaMa model
...
def node(state: GraphState) -> Dict[str, Any]:
    """
    LaMa inpainting node.
    
    UPDATED: Now saves inpainted image to tools_output directory.
    ...
    try:
        ...
        return r_pack_state(
            state,
            dequeue_update,
            r_set_layer_error(layer_id, f"LaMa tool import failed: {e}", {}, state)
        )
    ...
    # Run LaMa
    try:
        out_path = run_lama(image_path, mask_path)
    except Exception as e:
        return r_pack_state(
            state,
            dequeue_update,
            r_set_layer_error(layer_id, f"LaMa failed: {e}", {}, state)
        )
    ...
    if not out_path or not Path(out_path).exists():
        return r_pack_state(
            state,
            dequeue_update,
            r_set_layer_error(layer_id, "LaMa output not found", {}, state)
        )
    
    # Copy output to tools_output directory
    final_output_path = tools_output_dir / "lama_inpaint.png"
    shutil.copy(out_path, final_output_path)
    
    # Also copy the mask used
    mask_copy_path = tools_output_dir / "lama_mask_used.png"
    shutil.copy(mask_path, mask_copy_path)
    
    output = {
        "tool_name": "inpaint_lama",
    ...
    output_json_path = tools_output_dir / "lama_output.json"
    with open(output_json_path, "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)
    
    print(f"[LaMa] Inpainted {layer_id} -> {final_output_path}")
