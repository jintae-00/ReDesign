...
def node(state: GraphState) -> Dict[str, Any]:
    ...
    # print(f"[Object Clear] args : {args}")
    out_img = _call_tool("ObjectClear", args, state)

    pieces: list[Dict[str, Any]] = [upd0]

    # 1) Save artifact
    u_art, saved = r_save_artifact(out_img, "objectclear", "inpaint", state)
    pieces.append(u_art)

    # 2) Update inpaint (sanitize to prevent overwriting artifacts)
    u_inp = r_update_inpaint("objectclear", saved, state)
    pieces.append(u_inp)

    # 3) Refresh the base slot
    try:
