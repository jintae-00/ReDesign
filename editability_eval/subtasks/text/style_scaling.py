...
def run(
    ...
    scales: Sequence[float] = (1.6, 2.2, 3.0, 4.0),
    ...
    candidates = build_style_candidates(pairs, "super_scaling", [{"scale": float(s)} for s in scales])
    results = evaluate_style_subtask(
        candidates,
        cache,
        seed=seed,
        max_tasks=max_tasks,
        progress_prefix=f"[{model}][style_scaling]",
        log_every=log_every,
        save_pair_viz_dir=(output_dir / model / "style_scaling" / "element_pairs") if save_pair_viz else None,
        pair_viz_max=pair_viz_max,
        reference_results=reference_results,
        num_workers=num_workers,
        show_tqdm=show_tqdm,
        checkpoint_path=output_dir / model / "style_scaling_results.json",
        resume=True,
    )
    return {
